...
from app.applications.domain.exceptions import ProfileNotComplete, ApplicationAlreadyExists
from app.applications.domain.usecases.create_new_application_use_case import CreateNewApplicationUseCase
...
class TestCreateNewApplicationUseCase(ApiTests):
    ...
    def test__given_a_user_with_all_the_information_and_non_existing_event__when_create_application__then_event_not_found_is_raised(
        ...
        with self.assertRaises(EventNotFound):
            CreateNewApplicationUseCase().execute(token=self.user_complete_token, event_id=uuid.uuid4())
    ...
    def test__given_non_existing_user_id_and_existing_event__when_create_application__then_user_not_found_is_raised(
        ...
        with self.assertRaises(UserNotFound):
            CreateNewApplicationUseCase().execute(token=uuid.uuid4(), event_id=self.event_id)

    def test__given_a_user_without_some_information__when_create_application__then_profile_not_complete_is_raised(self) -> None:
        # Given
        ...
        with self.assertRaises(ProfileNotComplete):
            CreateNewApplicationUseCase().execute(token=user_token, event_id=self.event_id)
    ...
    def test__given_a_user_with_all_the_information_and_a_existing_event__when_create_application_two_times_with_the_same_information__then_application_already_exists_is_raised(
        ...
        # When / Then
        CreateNewApplicationUseCase().execute(token=self.user_complete_token, event_id=self.event_id)
