...
class UpdateUserUseCase:
    ...
    def execute(self, token: uuid.UUID, user_data: UpdateUserRequest) -> User:
        ...
        if user_data.work and (
            user_data.current_job_role is None
            and original_user.current_job_role is None
        ):
            raise MissingWorkInformationToCreateUser

        if user_data.study and (
            (user_data.university is None and original_user.university is None)
            or (user_data.degree is None and original_user.degree)
            or (
                user_data.expected_graduation is None
                and original_user.expected_graduation is None
            )
        ):
        ...
        new_user = User(
            ...
            else original_user.first_name,
            last_name=user_data.last_name
            if user_data.last_name
            else original_user.last_name,
            username=user_data.username.lower()
            if user_data.username
            else original_user.username,
            ...
            else original_user.date_of_birth,
            study=user_data.study
            if user_data.study is not None
            else original_user.study,
            work=user_data.work if user_data.work is not None else original_user.work,
            university=user_data.university
            if user_data.university
            else original_user.university,
            degree=user_data.degree if user_data.degree else original_user.degree,
            expected_graduation=datetime.strptime(
                user_data.expected_graduation, "%d/%m/%Y"
            )
            if user_data.expected_graduation
            else original_user.expected_graduation,
            current_job_role=user_data.current_job_role
            if user_data.current_job_role
            else original_user.current_job_role,
            tshirt=TShirtSizes[user_data.tshirt]
            if user_data.tshirt
            else original_user.tshirt,
            gender=GenderOptions[user_data.gender]
            if user_data.gender
            else original_user.gender,
            alimentary_restrictions=user_data.alimentary_restrictions
            if user_data.alimentary_restrictions
            else original_user.alimentary_restrictions,
            github=user_data.github if user_data.github else original_user.github,
            linkedin=user_data.linkedin
            if user_data.linkedin
            else original_user.linkedin,
